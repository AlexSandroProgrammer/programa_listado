...
$comando = $connection->prepare("SELECT * FROM datetime_entry INNER JOIN user INNER JOIN type_user ON datetime_entry.document = user.document AND user.id_type_user = type_user.id_type_user WHERE user.id_type_user= 3 ORDER BY id_entry  DESC LIMIT 5");
$comando->execute();
...
<head>
    ...
    <title>ACTIVIDADES RECIENTES CLIENTES</title>
